...
재배열표 사용자 대조(C2)를 시도했으나 질문 전달이 끊겨 **미대조** 상태로 확정 진행한다(계획의 저위험 모호성 처리 원칙 적용 — 재구성이 TOC 원문 앵커 기반 신뢰도 H이고, 남은 모호성은 페이지 내 위치라는 지엽 사항뿐). 사용자는 PR 리뷰 시 §2 증거표를 원본과 대조해 이의를 제기할 수 있고, 수정 시 01만 고치면 된다(02는 단원 구조가 아니라 §3 추출 아키텍처와 §4 결정표에 의존).

## 6. 02(마스터플랜)로 넘기는 핵심 입력
